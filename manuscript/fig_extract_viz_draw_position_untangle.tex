\begin{figure*}[!htb]
    ...
    \begin{tabular}[t]{c}%{|c|c|}
        ...
        \begin{subfigure}[t]{1.0\textwidth}
            ...
            \label{fig:viz_extract_untangle-MHC}
        \end{subfigure} \\
        \begin{tabular}{c}% if you add [t], then sub images are pushed down
            ...
            \begin{subfigure}[t]{0.5\textwidth}
                ...
                \label{fig:viz_extract_untangle-C4-default}
            \end{subfigure} \\
            \begin{subfigure}[t]{0.5\textwidth}
                \centering
                \caption{}
                ...
                \label{fig:viz_extract_untangle-C4-gradient}
            \end{subfigure} \\
            \begin{subfigure}[t]{0.5\textwidth}
                \centering
                \caption{}
                ...
                \label{fig:viz_extract_untangle-C4-strandness}
            \end{subfigure} \\
            \begin{subfigure}[t]{0.5\textwidth}
                \centering
                \caption{}
                ...
                \label{fig:viz_extract_untangle-C4-coverage}
            \end{subfigure}
        \end{tabular}
        %&
        \begin{tabular}{c}% if you add [t], than sub images are pushed down
            ...
            \begin{subfigure}[t]{0.5\textwidth}
                ...
                \caption{}
                \label{fig:viz_extract_untangle-C4-bandage}
            \end{subfigure}
            \\\\
            \begin{subfigure}[t]{0.5\textwidth}
                \centering
                ...
                \caption{}
                \label{fig:viz_extract_untangle-C4-bandage-zoom}
            \end{subfigure}
        \end{tabular}\\
        \begin{subfigure}[t]{1.0\textwidth}
            \centering
            ...
            \begin{center}
                ...
                \label{fig:viz_extract_untangle-C4-untangle}
            \end{center}
        \end{subfigure} \\
        %\hline
    \end{tabular}
    \caption{
        ...
        The red rectangle highlights the C4 region.
        \textbf{(b-e)} \textit{odgi viz} visualizations of the C4 pangenome graph, where 8 paths are displayed: 2 reference genomes (\REVIEWED{CHM13} and \REVIEWED{GRCh38} on the top) and 6 haplotypes of 3 diploid individuals.
        \textbf{(b)} \textit{odgi viz} default modality: the image shows a quite linear graph.
        The links at the bottom indicate the presence of a structural variant (long link) with another structural variant nested inside it (short link on the left).
        \textbf{(c)} Color by path position. %: the color gradients are smooth, highlighting that its node are well sorted in 1 dimension.
        The top two reference genomes and one haplotypes (HG01952\#2) go from left to right, while 5 haplotypes go in the opposite direction, as indicated by the black color on their left.
        ...
        \textbf{(f)} \REVIEWED{Layout of the C4 pangenome graph made with the \textit{Bandage} tool~\citep{Wick_2015} and} annotated by using \textit{odgi position}. Green nodes indicate the C4 genes (in red). The red rectangle highlights the regions where \textit{C4A} and \textit{C4B} genes differ.
        ...
        \textbf{(h)} Visualization of \textit{odgi untangle} output in the C4 pangenome graph: the plots show the copy number status of the sequences in the C4 region with respect to the \REVIEWED{GRCh38} reference sequence, making clear, for example, that in HG00438\#2, the \textit{C4A} gene is missing (no black lines in the region annotated in red). \vspace{-1em}
    }
    \label{fig:odgi_viz}
\end{figure*}
