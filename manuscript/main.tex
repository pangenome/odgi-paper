...
% Confessions
% Future plans
Pangenome graphs stand to become a ubiquitous model in genomics~\citep{Eizenga_2020}.
With ODGI we implemented a state-of-the-art tool suite that enables users to explore and discover the underlying biology in pangenomes graphs, filling the gap that made pangenomic analyzes difficult.
Indeed, with ODGI's tools users can easily transform, analyse, simplify, validate, and visualise pangenome graphs at large scale.
...
Nevertheless, ODGI's graph layout algorithms could become a backbone of such tools, as is already the case for \textit{gfaestus}~\citep{gfaestus} %\footnote{\url{https://github.com/chfi/gfaestus}},
which currently relies on the \textit{odgi layout} output for its interactive 2D visualization.
