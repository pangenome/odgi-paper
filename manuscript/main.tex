...
We measured the efficiency of ODGI (VERSION!) and VG (VERSION) for routine pangenome tasks. In particular, we measured the execution time and memory usage 
\begin{inparaenum}[(i)]
	\item of transforming a GFAv1 into the tool's native format,
	\item the visualization of a pangenome graph, 
	\item the extraction of a subgraph, and
	\item the finding of path positions in a pangenome graph.
\end{inparaenum}
key graph operations, why
deNBI
data
threads (64haps), haps (16 threads)
general findings: VG sucks, hehehe
