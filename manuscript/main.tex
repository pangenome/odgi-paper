...
These flexible models let us encode any kind of variation, allowing the generation of truly unbiased \FIXME{unbiased needs a little explanation} data systems on which to base analyses of genome evolution.
...
As experience \FIXME{this is a bit odd} in the HPRC has shown, pangenome graphs are a natural model to support these kinds of analyses, but studies based on pangenome graphs are difficult due to a lack of downstream tools and standard operations.
...
This standardization supports the development of a reference set of tools that operate on the pangenome graph model, while the need to understand the graphs made by these methods motivates it. \FIXME{Sentence is a bit odd}.
...
\item They contain paths are locally similar to each other.
...
\FIXME{without a figure we may lose the reader here a little}
...
To seamlessy represent such difficult regions, we follow \FIXME{or introduce?} the approach of the dynamic Graph BWT (GBWT) \citep{Siren:2020}, and built a node-centric, dynamic, compressed model of the paths.
\FIXME{So this has never been done before?}
...
\MyStruct{Node}{
    ...
    \textbf{sequence} $= [$A$|$T$|$G$|$C$]*$ \tcp{DNA, RNA, protein}
    ...
    \textbf{path\_steps} $[Step_1 \ldots Step_\Omega]*$
}
\MyStruct{Step}{
...
For efficiency $Node_i.sequence$ is stored as a plain string, while the $edges$ and $path\_steps$ are stored using a dynamic succinct integer vector that requires $O(2nw)$ bits for the edges and $O(5nw)$ bits for the path steps, where $w$ is $\approx log_2(n)$ \citep{prezza2017framework}.

To allow edit operations in parallel, each node structure includes a byte-width mutex $lock$.
All edit operations on the graph must touch at most two $Node$ structs at a time.
(Both our edge and path step representations are doubly-linked.)
...
This ``atomic'' approach to edit operations on the graph is key to enabling our method to scale to the largest genome graphs that we can currently build.
\FIXME{This is great!}
...
\FIXME{why not just say human, mouse, \ldots?}
...
A few design considerations shape these tools.
To navigate the graph, we need a coordinate system.
ODGI uses the embedded pangenome sequences to provide a set of coordinates in a standard, 1D format.
These coordinates depend on the sequences, and are graph-independent.


To provide a universal coordinate space for the graph, we project its nodes into a low-dimensional space (in practice, 1 or 2 dimensions) by learning the projection via a HOGWILD!, path-guided stochastic gradient descent (PG-SGD) model.
\FIXME{Cite or explain?!}
...
\FIXME{Would it be an idea to put part of pggb in a box and explain steps? That way people can visualize using the toolkit and we can refer to it in the text}
...
\texttt{odgi view} can output GFA and internal graph representations \\
\texttt{odgi stats} outputs statistics in TSV or YAML. \\
\texttt{odgi bin} can output JSON. \\
\texttt{odgi degree/position/depth/untangle/tips} can output BED. \\
\texttt{odgi untangle} outputs PAF.
