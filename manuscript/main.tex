...
We pursue a novel approach to visualization with \textit{odgi viz} and \textit{odgi draw}, two tools that provide scalable ways of generating \REVIEWED{static} pictures of the high-level structure of even large pangenome graphs.

\textit{odgi viz} supports a binned, linearized rendering in 1 dimension (1D) (i.e., all graph nodes lie on the same axis).
...
By default, path colors are derived from the path names (Fig.~\ref{fig:viz_extract_untangle-C4-default}).
...
\textit{odgi draw} extends the visualization in 2 dimensions (2D)  (Fig.~\ref{fig:viz_extract_untangle-MHC}) by rendering the layout built by \textit{odgi layout} (\S\ref{sec:sort}). \REVIEWED{It took \textit{odgi layout} and \textit{odgi draw} 6 minutes and 21 seconds in total to produce the 2D layout of the MHC locus (Fig.~\ref{fig:odgi_layout}).}
...
The results of other useful graph editing operations are described in $\S$\ref{sec:supp_edit}.
