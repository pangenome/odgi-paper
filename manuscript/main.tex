...
With \textit{odgi prune} users can remove complex parts as defined by query criteria, while with \textit{odgi break} they can remove cycles in the graph, reducing the complexity of the graph topology.
...
Moreover, to obtain base-level precise information on the relationships between the repeated sequences, we can use any sequence aligner to align the pairs of regions that came from the untangling (as {cite wfmash}).
