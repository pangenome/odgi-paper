...
In more detail, we find that it is necessary to support graphs with regions with very high numbers of path traversals (high path depth).
...
If we cannot process such regions, there are only two options: 1) we can remove such regions, or 2) we can leave them unaligned.
