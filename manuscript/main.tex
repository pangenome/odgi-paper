...
\begin{document}
    ...
    \title[short Title]{ODGI: scalable tools for pangenome graphs}
    ...
    \abstract{ \textbf{Motivation:} Pangenome graphs provide a
      complete and `lossless' representation of genomic information
      that allows for complex population-sized analysis of genomes
      without introducing reference genome bias. Here we present a
      range of novel tools that makes use of the \odgi\ efficient
      in-memory representation of DNA/RNA graphs. \odgi\ can handle
      thousands of large genomes while maintaining fast execution
      runtime and low memory overhead. \odgi\ includes tools for
      detecting complex regions, extracting loci, exploratory
      analysis, sorting, removing artifacts and visualisation.
      %
      \\
      \textbf{Availability:} \odgi\ is software written in the \Cpp\
      programming language and published under the libre MIT license. Source
      code and user manual can be downloaded from
      ...
      %
        ...
        \href{erik.garrison@gmail.com}{erik.garrison@gmail.com}\\ \textbf{Supplementary
...
%        \end{table}
    ...
    \cmd{odgi viz}: by visualizing pangenome graphs, we can gain insight into the relationship between genomes. \texttt{odgi viz}
