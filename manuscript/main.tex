...
Similar to other efficient libaries presenting the \textsc{HandleGraph} model~\citep{Eizenga_2020_BX}, the implementation of ODGI's tools rests on three key properties which hold for most pangenome variation graphs:
...
\item \textit{odgi position} lifts coordinates between paths and graph positions (\S\ref{sec:untangle}).
...
\subsection{Converting to standard file formats{\color{red} FIXME: We need a different title/subsection}
}
\label{sec:text}
...
{\color{red} FIXME: We should put the following information in the metric section, of move some commands here}
...
\textit{odgi server} implements a basic HTTP server which can translate a 'path:position' to a 'pangenome:position' very efficiently. {\color{red} FIXME: this command is listed in the list in 4 Results}
