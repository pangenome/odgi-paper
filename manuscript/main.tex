...
In contrast to reference-based approaches which relate sequences to a single genome, these data structures encode the mutual relationships between all the genomes \REVIEWED{[1,2]and genomic sequences[1,2]} represented.
...
A bidirected sequence graph can \REVIEWED{[1,2] represent both strands of the genomic sequences of the DNA. [1,2]}
...
Although \REVIEWED{pangenome graphs} are data structures of utility to researchers~\citep{cpang2018,Garrison:2018,Baaijens_2019,Hickey:2020,Sibbesen_2021}, the scientific community still lacks a toolset \REVIEWED{capable of operating on gigabase-scale pangenomes built from whole-genome assemblies}.
...
\MyStruct{Node}{
    ...
    \textbf{sequence} $= [$A$|$T$|$G$|$C$|$N$]*$ \REVIEWED{[1,2] \tcp{genomic}}
...
This construction step can be a significant bottleneck, in particular as the size of the path set of the graph increases. \REVIEWED{[1,5] The process itself is lossless. A graph in ODGI format represents everything that is in the input GFAv1 graph, without any loss of information. [1,5]}
...
Pangenome graphs represent both strands \REVIEWED{[1,2] of the genomic sequences of the DNA. [1,2]}
...
Navigating and understanding the graph requires coordinate systems that we can use to link other data into the graph model, and thus to all genomes \REVIEWED{[1,2]and genomic sequences[1,2]} in the pangenome.
