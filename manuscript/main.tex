...
\MyStruct{Node}{
    ...
    \textbf{edges} $= (s_{x},s_{y})* : x \land y \in [1\ldots N]$ \\
    \tcp{bit-packed vector of id deltas}
    \textbf{decoding} $x_i \ldots x_N \in \mathbb{N}^N$ \\
    \tcp{bit-packed vector of path steps}
    \textbf{paths} $[Step_1 \ldots Step_M]*$
}
\MyStruct{Step}{
    ...
    \textbf{prev$_\delta$} $\in [1\ldots N]$ \tcp{$\delta$-encoded previous node}
    \textbf{prev\_rank} $\in \mathbb{N}$ \tcp{step rank on previous node}
    \textbf{next$_\delta$} $\in [1\ldots N]$ \tcp{$\delta$-encoded previous node}
