...
\begin{document}
    ...
    \begin{table}[!t]
      \processtable{Example of performance improvement between
        \cmd{odgi extract} and \cmd{VG something}. Centromere region
        is extracted from CHR 6 of the HPRC year 1
        assembly\citep{HPRC}, i.e., 88 haploid, phased human genome
        assemblies from 44 individuals plus the chm13 and GRCh38
        reference genomes stored in 4.3GB GFA format. All timings
        were performed on a server class machine on SSD MODEL with X
        GB of RAM and 48 CPU cores (Y x 12 core AMD Opteron) Processor
        MODEL @ 3.3 GHz with 8MB L2 Cache.\FIXME
          \label{tab:02}} {
