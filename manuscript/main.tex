...
\subsection{Path Jaccard Index}
\textit{odgi tips}, \textit{odgi untangle}, and \textit{odgi position} all need to translate a query path position to a target path position. In a repeat region, it could be that the node of the query path position is visited several times by the target path. We use the graph \textit{path jaccard} concept to infer the best target position for a query path position. For each query position we look at each possible target window and find the most-similar one. 
 The \textit{path jaccard} context mapping ensures a precise position translation process. It explores the nucleotide context 
