...
The ODGI toolkit builds on existing approaches to efficiently store and manipulate variation graphs \citep{Garrison:2018,Eizenga_2020_BX}. %, presenting a refined and well-tested implementation.
...
%These properties and this  encourage us to use a node-centric model for the graph, where we apply a local, compressed representation of the node neighborhood.

%These structures are complex, but we can make sense of them so long as we can load and analyze   %In order to make sense of such structures
%(Although we usually seek to remove as many such motifs as possible through graph normalization, to do so requires that we be able to load and traverse the graph.)
%Our objective is to allow dynamic modification of graphs even when they contain nodes with very high path depth.
...
%wherein we save space using a local encoding of paths 
%We construct a node-centric model of the graph.
