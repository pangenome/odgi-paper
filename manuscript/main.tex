...
If we cannot process such regions, \REVIEWED{we cannot understand them}, and there are only two options: 1) remove such regions, or 2) leave them unaligned.
Neither of these solutions allows us \REVIEWED{to evaluate} their biological \REVIEWED{or technical} features.
\REVIEWED{Such approaches presuppose that we are unable to understand variation in these regions.}
...
\REVIEWED{Pangenome graphs built from hundreds of haplotype-resolved \textit{de novo} genome assemblies are very large, but it is often necessary to work with only a small portion of the genomes represented,
