...
\REVIEWED{The the major histocompatibility complex (MHC) locus is located on the human chromosome 6 and it encodes molecules involved in antigen presentation, inflammation regulation, the complement system, and the innate and adaptive immune responses~\citep{Shiina2009}.
MHC genes are highly polymorphic, i.e., there are multiple different alleles across individuals in a population.
Such variability becomes evident when we apply \textit{odgi draw} to visualize the graph layout of a human MHC pangenome graph (Fig.~\ref{fig:viz_extract_untangle-MHC}).
The visualization displays the graph topology in 2 dimensions (2D) and how the paths traverse the graph, with the regions of variations that appear as bubbles in the layout.
The layout was built by \textit{odgi layout} (see \S\ref{sec:sort}).} Of note, a 2D rendering can be costly to compute, but we provide an implementation that scales linearly with pangenome sequence size, allowing us to apply it to large pangenome graphs.
...
In Fig.~\ref{fig:viz_extract_untangle-MHC}, such a region corresponds to the little bubble highlighted by the red rectangle.
To take a closer look, we obtained a C4 pangenome graph by extracting the C4 region from the full MHC pangenome graph (see \S\ref{sec:extract}).
Then, we visualized such a subgraph by applying} \textit{odgi viz}, which produces binned, linearized renderings in 1 dimension (1D) (i.e., \REVIEWED{each path lies} on a single axis) (Fig.~\labelcref{fig:viz_extract_untangle-C4-default,fig:viz_extract_untangle-C4-gradient,fig:viz_extract_untangle-C4-strandness,fig:viz_extract_untangle-C4-coverage}).
...
\REVIEWED{In humans, it exists as 2 functionally distinct genes, \textit{C4A} and \textit{C4B}, which both vary in structure and copy number~\citep{Sekar_2016}.
...
Moreover, the short nested link on the left highlights that \textit{C4A} and \textit{C4B} genes segregate in both long and short genomic forms, distinguished by the presence or absence of a human endogenous retroviral (HERV) sequence.}
