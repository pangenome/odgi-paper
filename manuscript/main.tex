...
For example, we find that it is necessary to support graphs with regions with very high numbers of path traversals (high path depth).
...
ODGI thus provides a set of interfaces that let us approach these graphs from the perspective of standard reference- and sequence-based data models.
...
\textit{odgi flatten} generate a linearization of the graph by emitting the pangenome sequence (the concatenation of all graph node sequences) in FASTA format, and the projection of all paths on the linearized sequence in BED format.
...
Recent interactive browsers are reference centric~\citep{Beyer2019, Yokoyama2019, Durant2021, Liang2021} or focusing on 2D~\citep{Wick_2015, Gonnella2018}.
