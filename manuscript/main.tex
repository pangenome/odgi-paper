...
\begin{document}
    ...
    provides convenient insight into genomic variation (see
    Fig.~\ref{fig:1}). \cmd{odgi viz} generates a linearized
    representation of the pangenome and is capable of handling full
    length human chromosomes (Fig.~\ref{fig:1}a). \cmd{odgi sort}
    reorganises node order and simplifies the graph by applying
    several algorithms, including the novel `Path-Guided Stochastic Gradient Descent`
    algorithm. \cmd{odgi layout} and \cmd{odgi draw} bring the
    visualization in 2D. (Fig.~\ref{fig:1}b).

    \topic{odgi stats, bin, depth \& degree} pangenome statistics allow exploring genome complexity at different
    levels of abstraction. \cmd{odgi stats} returns the number of nodes, edges, paths, and graph length, while
    \cmd{odgi bin} summarizes the path information into bins of specified size, enabling a summarized view of
    gigabase scale graphs. \cmd{odgi depth} and \cmd{odgi degree} return the node depth and degree, respectively, as
    defined by query criteria, allowing users to detect complex regions due to sequences with highly identical
    repeats, or problems in the pangenome graph.
