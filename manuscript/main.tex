...
Pangenome graphs are a natural model to support these kinds of analyses, but studies based on pangenome graphs are difficult due to a lack of downstream tools and standard operations.
...
Most read or write the native ODGI format (.og) (Figure \ref{fig:operations}) and work with standard text based data formats common to bioinformatics.
...
% build
We begin by transforming the ``columnar'' storage model of GFA, in which nodes, edges, and paths are described independently, into the ODGI node-centric encoding with \textit{odgi build}.
...
The ODGI data structure (Algorithm \ref{alg:structs}) allows algorithms to dynamically construct the graph and its path set to operate in parallel.
In \textit{odgi build}, we initially construct the node vector in a serial operation that scans across the input GFA.
Edges, and then paths, can be added in parallel by obtaining mutex $Node.lock$ for pairs of nodes and adding edges or steps to their corresponding vectors in both nodes.
