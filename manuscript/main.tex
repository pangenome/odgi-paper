...
\REVIEWED{Using \textit{odgi extract}, we extracted the major histocompatibility complex (MHC) locus from a 90-haplotype human chromosome 6 pangenome graph form the HPRC.
...
Furthermore, we can use multiple color palettes to color the paths \REVIEWED{by how many times they traverse a node, which can be referred to as the path's depth or coverage of the node.}
...
The graph is then updated in place. First, the node identifiers are normalized (from 1 to the number of nodes) including the adjustment of the edges.
Second, path information, specifically each step of a path, is updated, too.
