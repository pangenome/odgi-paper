...
\item \textit{odgi view} converts the ODGI data model into GFA (\S\ref{sec:text}).
...
By considering all paths in the graph as potential reference or query sequence, we make the graph invisible to downstream tools that operate on collections of sequences (\textit{e.g.} BEDTools, SAMtools~\citep{Li2009}, aligners).
