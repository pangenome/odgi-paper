...
\begin{document}
    ...
    \abstract{ \textbf{Motivation:} Pangenomes graphs address some of the
      ...
      structural variants.
        ...
        large genomes in memory with fast
        parallel execution.  \odgi\ includes
        tools for detecting complex regions, extracting \textit{loci},
        exploratory analysis, sorting, removing artifacts, and graph
        manipulation and visualisation. \odgi\ also provides an extensible API that allows
        for writing new tool.
        %
        \\
    ...
    genome assemblies are being produced at a high rate, including
    ...
    orthologs. So-called paths represent reads, contigs, haplotypes, individual chromosomes or part thereof, that run through the
    ...
    exploring complex variation and highly repetitive regions.
    ...
    graph structure in computer memory that can be dynamically updated using
    ...
    (see Fig.~\ref{fig:1}a). \cmd{odgi sort} reorganises node order and
    simplifies the graph by applying several algorithms, including the
    novel `path-guided stochastic gradient descent' algorithm.
    \cmd{odgi layout} generates a layout file with the X-Y coordinates for
    all nodes and \cmd{odgi draw} draws it, bringing the pangenome representation
    in two dimensions.
    ...
    in the graph and any contained path can be a reference.
    ...
    visualise pangenome graphs at scale.  \odgi\ also exposes an extensible API
    that can be used by other tools, e.g., for genotyping, pangenome
    graph building and normalization. Future work will add support for
    RNA and protein sequences and expand on metadata capabilities of
    large pangenome graphs.
